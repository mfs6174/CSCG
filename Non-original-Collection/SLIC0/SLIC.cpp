	...
	const int&					NUMITR)
{
	...
	while( numitr < NUMITR )
	{
		...
		for( int n = 0; n < numk; n++ )
		{
			...
			int y2 = min(m_height+0.0,	kseedsy[n]+offset);
			int x1 = max(0.0,			kseedsx[n]-offset);
			int x2 = min(m_width+0.0,	kseedsx[n]+offset);
			...
			for( int y = y1; y < y2; y++ )
			{
				...
				{
					int i = y*m_width + x;
					//CV_ASSERT( y < m_height && x < m_width && y >= 0 && x >= 0 );
...
int SLIC::SLICO_Provider(Mat input,vector< vector<int> >  &slicLabels,Mat &output,int K,int &numLab)
{
  //uint alpha r g b (4*uchar=uint)
  ...
  if(m_spcount > sz)
  {
    cout<<"Number of superpixels exceeds number of pixels in the image"<<endl;
    return -1;
  }
  ...
  for (int i=0;i<height;i++)
    ...
    {
      int idx=i*width+j;
      Vec3b &vv=input.at<Vec3b>(i,j);
      UINT t0=0;
      img[idx]= (t0|( ( (UINT)vv[2])<<16)| ( ( (UINT)vv[1])<<8)|((UINT)vv[0]));
    }
  ...
  for (int i=0;i<height;i++)
    ...
    {
      int idx=i*width+j;
      slicLabels[i][j]=labels[idx];
      Vec3b &vv=output.at<Vec3b>(i,j);
      vv[0]=img[idx] & 0xFF;
      vv[1]=(img[idx]>>8) & 0xFF;
      vv[2]=(img[idx]>>16) & 0xFF;
    }
